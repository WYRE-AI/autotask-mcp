...
- **The `autotask_{create,update,delete}_*` naming convention is now load-bearing for security, not just tidiness.** `isWriteTool()` in `src/utils/write-policy.ts` classifies tools by that prefix, so a mutating tool named anything else (`autotask_close_ticket`, `autotask_merge_companies`) would be invisible to read-only mode and execute against a live PSA. `tests/read-only-mode.test.ts` pins this: every shipped tool must be verb-prefixed (`create|update|delete|search|get|list`) or appear in an explicit `REVIEWED_EXCEPTIONS` list. If that test fails, the fix is to name the tool conventionally or consciously add it to the exception list — never to loosen the assertion.
- **Hiding a tool from `tools/list` is not an access control.** Clients can and do call tool names they already know, so the read-only gate is enforced in `callTool`'s dispatch path; the `listTools` filter is ergonomics layered on top. The same reasoning covers the three indirect routes that would otherwise be trivial bypasses: `autotask_execute_tool` (re-checks the inner tool), `autotask_raw_request` (restricted to `GET` — it is the untyped REST escape hatch, so a `POST` through it is a full write), and the discovery meta-tools.
- **Deny-by-default on unknown input.** `autotask_raw_request` refuses a *missing or unrecognized* `method` rather than treating it as a read. An allowlist/denylist that fails open on malformed input is the usual way these gates get bypassed.
- **When adopting from a downstream fork, diff against current `main` before implementing.** Of the four changes taken from `Loffler-NOC/autotask-mcp`, the `ne` → `noteq` operator fix and the `queueID` filter had already landed here independently (`autotask.service.ts:449`, `:452`). Same lesson as the #237 entry above: scope the ask against the code, not the description.
