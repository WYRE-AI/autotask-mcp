...
- **Autotask silently drops query clauses it won't honour**, so a filter on a nonexistent field returns *everything* looking like a filtered result. Bitten twice: the `ne`/`noteq` operator bug (#193) and a `field: 'projectID'` filter against `TimeEntries` (#277). Silent wrong data is the worse failure mode — when touching a filter, assert on the **query body actually sent**, not just the return value.
