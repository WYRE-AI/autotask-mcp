...
- **🔒 SECURITY: Cross-tenant data leak via `MappingService` static singleton.** In gateway mode, the autotask-mcp sidecar serves multiple tenants from a single process. `MappingService` exposed a static `getInstance()` backed by a class-level `initPromise` — the FIRST request to call it permanently bound the resulting MappingService (with its company / resource ID-to-name cache) to whichever tenant's `AutotaskService` warmed it. Every subsequent request — including requests from DIFFERENT tenants — received the same instance and read the first tenant's cached names. Symptom: a tenant's tickets / projects came back with `company` and `assignedTo` fields populated from another tenant's directory (engineers, project names) because `enhanceItems()` resolved IDs against the wrong tenant's cache.
  ...
  - `AutotaskToolHandler.getMappingService()` now calls `MappingService.create(...)`. Because `AutotaskToolHandler` is already constructed per-request in gateway mode (via `McpServer.buildPerRequestHandlers`, added in the worker refactor), each request now ends up with its own isolated `MappingService` whose cache cannot bleed across tenants.
  - Gateway-mode `/mcp` now rejects requests missing any of `X-API-Key` / `X-API-Secret` / `X-Integration-Code` with HTTP 401 (JSON-RPC error `-32001`) instead of falling through to the env-configured `this.toolHandler`. Previously a header-less request in gateway mode would have been served by the operator's env-configured tenant — a different shape of cross-tenant exposure.
  - 4 new regression tests in `tests/mapping.test.ts` under `tenant isolation (cross-tenant leak regression)`: independent instances per tenant; no cross-pollution under concurrent init; clearing one tenant's cache leaves another untouched; 10-tenant high-fan-out parallel-init each-sees-only-own-data assertion. The `should return the same instance on subsequent calls` test (which encoded the vulnerable singleton behavior as expected) was inverted to `should return a DISTINCT instance on each call (no cross-tenant leak)`.
