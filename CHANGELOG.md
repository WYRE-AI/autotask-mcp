...
- **Contact updates now fall back to the `Companies/{companyID}/Contacts` child route on zones that reject both standard update routes** ([#197](https://github.com/wyre-technology/autotask-mcp/pull/197), follow-up to [#133](https://github.com/wyre-technology/autotask-mcp/issues/133)). Some zone hosts exhibit the #133 behaviour (collection-level `PATCH /Contacts` returns an IIS HTML **404**) but *also* reject the #133 fallback `PUT /Contacts/{id}` with **405** — reported and verified in production against a US zone by @pdlaskbis, so the misregistration is not limited to DE1. On those zones both legs of `AutotaskHttpClient.update()` fail and contact updates are impossible through the typed tool.
  - `AutotaskService.updateContact()` now retries through `PATCH /Companies/{companyID}/Contacts` — the update path Autotask documents for child entities — resolving the parent `companyID` from the update payload when supplied, otherwise via `getContact(id)` (one extra GET, only on already-failing zones).
  - The child route is strictly a **last-resort fallback**, gated on 404/405 from `update()`: healthy zones keep their single-request `PATCH /Contacts` path, DE1 keeps its verified `PUT` fallback, and genuine validation errors (400/422) surface unchanged. The original #197 proposal routed *all* contact updates through the child route unconditionally, which risked regressing DE1 — the #133 changelog explicitly flagged nested child-route PATCH as unverified there.
  - `companyID` 0 (the MSP's own root company) is treated as a valid parent — the resolution checks for `undefined`/`null`, not falsiness.
  - 7 new tests in `tests/contact-update-child-route.test.ts` pin the fallback ordering (PATCH → PUT → GET → child PATCH), the payload-supplied and looked-up `companyID` paths, the company-0 edge, the no-fallback-on-400 gate, and the unchanged healthy-zone/DE1 request sequences.
